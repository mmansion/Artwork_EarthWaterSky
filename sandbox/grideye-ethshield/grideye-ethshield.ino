...
void copyToCharArray() {
  for (int i = 0; i < 128; i++) {
    // Assuring that the values are between 0 and 3
    if (combinedPixelTable[i] >= 0 && combinedPixelTable[i] <= 3) {
      // Convert integer to char and store in charArray
      charArray[i] = '0' + combinedPixelTable[i];
    } else {
      // Handle error for values outside 0-3
      charArray[i] = 'E'; // storing 'E' to indicate error
    }
  }
